...
homogen_power_plot <- function(obj){
  ...
  if (obj$model == "fixed"){
    ...
    p <- ggplot(obj$homo_range %>% dplyr::filter(k_v == obj$k), aes(x = SD, y = power,group = k_v, color = k_v)) + geom_line(size = 1) +
      geom_point(aes(x = obj$sd, y = obj$homo_test), shape = 21, color = "black", fill = "red", size = 3)

  } else if (obj$model == "random"){

    p <- ggplot(obj$homo_range, aes(x = k_v, y = power, linetype = Heterogenity)) +
      geom_line(size = 1) +
