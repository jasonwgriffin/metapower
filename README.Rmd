...
7. Main effects
8. Between-study variance (for Random-effects models)
...
Given, this set of expected values, we have `r round(my_mod$power_b*100,2)`% to detect between-group differences between face perception and face recognition deficits in ASD based on these set of expectations. As expected, moderator effects are much harder to detect and more studies are required, especially when heterogenity is high.
