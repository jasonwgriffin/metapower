...
server <- function(input, output) {
  ...
  output$d_plot <- renderPlot({
    ...
    metapower::plot_mpower(mpower(effect_size, study_size, k, es_type, test_type, p))
  })
  output$d_summary <- renderPrint({
    ...
    p <- input$d_p
    test_type <- input$d_test_type
    print(mpower(effect_size, study_size, k, es_type, test_type, p))
  })
  ...
  output$homogen_d_plot <- renderPlot({
    ...
    p <- input$homogen_d_p
    metapower::plot_homogen_power(homogen_power(effect_size, study_size, k, es_type, p))
  })
  output$homogen_d_summary <- renderPrint({
    ...
    study_size <- input$homogen_d_n
    p <- input$homogen_d_p
    print(homogen_power(effect_size, study_size, k, es_type, p))
  })
  ...
  output$subgroup_d_plot <- renderPlot({
    ...
    metapower::plot_subgroup_power(subgroup_power(n_groups, effect_sizes, study_size, k, es_type, p))
  })
  ...
  output$subgroup_d_summary <- renderPrint({
    ...
    study_size <- input$subgroup_d_n
    p <- input$subgroup_d_p
    print(subgroup_power(n_groups, effect_sizes, study_size, k, es_type, p))
  })
  ...
  output$mod_d_plot <- renderPlot({
    ...
    p <- input$mod_d_p
    metapower::plot_mod_power(mod_power(n_groups, effect_sizes, study_size, k, es_type, p))
  })
  ...
  output$mod_d_summary <- renderPrint({
    ...
    study_size <- input$mod_d_n
    p <- input$mod_d_p
    print(mod_power(n_groups, effect_sizes, study_size, k, es_type, p))
  })
  ...
  output$or_plot <- renderPlot({
    ...
    metapower::plot_mpower(mpower(NULL,study_size, k, es_type, test_type, p, con_table))
  })
  output$or_summary <- renderPrint({
    ...
    test_type <- input$or_test_type
    con_table <- c(input$or_a, input$or_b, input$or_c, input$or_d)
    print(mpower(NULL,study_size, k, es_type, test_type, p, con_table))
  })
  ...
  output$homogen_or_plot <- renderPlot({
    ...
    metapower::plot_homogen_power(homogen_power(NULL,study_size, k, es_type, p, con_table))
  })
  output$homogen_or_summary <- renderPrint({
    ...
    p <- input$homogen_or_p
    con_table <- c(input$homogen_or_a, input$homogen_or_b, input$homogen_or_c, input$homogen_or_d)
    print(homogen_power(NULL,study_size, k, es_type, p, con_table))
  })
  ...
  output$subgroup_or_plot <- renderPlot({
    ...
    metapower::plot_subgroup_power(subgroup_power(n_groups, NULL, study_size, k, es_type, p, con_table))
  })
  ...
  output$subgroup_or_summary <- renderPrint({
    ...
    print(subgroup_power(n_groups, NULL, study_size, k, es_type, p, con_table))
  })
  ...
  output$mod_or_plot <- renderPlot({
    ...
    metapower::plot_mod_power(mod_power(n_groups, NULL, study_size, k, es_type, p, con_table))
  })
  ...
  output$mod_or_summary <- renderPrint({
    ...
    print(mod_power(n_groups, NULL, study_size, k, es_type, p, con_table))
  })
