...
The first part of the output shows the expected input values, where the main results are shown in the bottom portion, mainly, `Estimated Power`. Under this set of values, our power to detect a mean difference under a Fixed-Effects model is  `r round(my_power$power$fixed_power*100,2)`%. Furthermore, we can look at the power under a Random-Effects model under various heterogeneity levels (e.g., Low, Moderate, Large). For the output regarding `Estimated Power: Test of Homogeneity`, please see below
...
For Random-Effects models, `homogen_power` computes power across a range of possible heterogeneity values (i.e., *I^2*)
- *I^2* =  0% (No Heterogeneity)
- *I^2* = 25% (Low Heterogeneity)
- *I^2* = 50% (Moderate Heterogeneity)
- *I^2* = 75% (Large Heterogeneity)

We can visualize this across a range with `plot_homogen_power`
...
Given, this set of expected values, we have `r round(my_mod$subgroup_power$fixed_power_b*100,2)`% to detect between-group differences under a Fixed-Effects model. As expected, moderator effects are much harder to detect and more studies are required, especially when heterogeneity is high.
